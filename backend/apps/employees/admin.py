...
class EmployeeAdmin(admin.ModelAdmin):
    ...
    def full_name(self, obj):
        try:
            ...
            return str(obj.user.username) if obj.user else 'Unknown'
    ...
    def department_link(self, obj):
        if obj.department:
            try:
                url = reverse('admin:employees_department_change', args=[obj.department.id])
                return format_html(
                    '<a href="{}">{}</a>',
                    str(url),
    ...
    def total_shifts(self, obj):
        ...
        if count > 0:
            try:
                url = reverse('admin:employees_shift_changelist')
                return format_html(
...
class AttendanceAdmin(admin.ModelAdmin):
    ...
    def employee_link(self, obj):
        try:
            url = reverse('admin:employees_employee_change', args=[obj.shift.employee.id])
            name = obj.shift.employee.user.get_full_name() or obj.shift.employee.user.username
            return format_html(
                '<a href="{}">{} ({})</a>',
                str(url),
                str(name),
    ...
    def shift_link(self, obj):
        try:
            url = reverse('admin:employees_shift_change', args=[obj.shift.id])
            return format_html(
                '<a href="{}">{}</a>',
                str(url),
